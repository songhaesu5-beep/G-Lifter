...
export const AIExplainer = (function(){
  ...
  function analyze(st){
    // 1) 진단: 현재 해소 수준 + 시간 비교 (+운영 의미)
    let diagnosis;
    if(st.finished){
      diagnosis = '작업 완료: 해소율 ' + st.rate + '% · 지시 셔플링 ' + fmt(st.shuffleTotal) + '건 중 ' + fmt(st.resolved) + '건 해소.';
      diagnosis += st.pending > 0
        ? ' 잔존 ' + fmt(st.pending) + '건은 후속 출고 일정에 반영이 필요합니다.'
        : ' 모든 지시 셔플링이 해소되어 선적 지연 위험이 없는 상태입니다.';
      ...
      diagnosis = fmt(st.progress) + '대 처리 기준 해소율 ' + st.rate + '%는 지시 셔플링 ' + fmt(st.shuffleTotal) +
        '건 중 ' + fmt(st.resolved) + '건만 해소된 수준입니다.';
      diagnosis += ' 이 속도면 전체 완료까지 약 ' + fmt(etaHour) + '시간이 소요되어, 운영상 반나절 이상의 작업 시간이 필요합니다.';
    } else {
      diagnosis = '아직 처리된 지시가 없어 진단할 수 없습니다.';
    }
    // 시간 비교 (G-Lifter 5분/건 vs 인간 10분/건)
    if(st.humanMin !== undefined && st.t > 0){
      diagnosis += ' G-Lifter 소요 ' + fmt(Math.round(st.t)) + '분은 인간 동일 처리(' + fmt(st.humanMin) + '분) 대비 ' + fmt(st.savedMin) + '분 절약 — 하루 근무(480분) 기준 약 ' +
        fmt(Math.round(st.savedMin / 480 * 100)) + '%의 인력 시간을 줄이는 효과입니다.';
    }
    ...
    if(topCell){
      ...
      cellAnalysis = topCell.id + ' 셀 — 이 셀에 셔플링 차량 ' + fmt(topCell.count) + '대가 출고 경로를 막고 있습니다. ' +
        ...
        topCell.id + '를 먼저 처리하면 전체 해소율이 ' + pctUp + '%p 오릅니다 — 이 셀 우선 투입 시 병목 해소로 후속 차량 흐름이 개선됩니다.';
    } else {
    ...
    if(st.shuffleTotal === 0){
      ...
      if(st.pending > 0){
        ...
        prediction += ' 잔존 ' + fmt(st.pending) + '건은 ' + hotCells + '에 집중돼 있어, G-Lifter를 2대 추가하면 예상 해소율 ' + addRate + '%까지 올라갑니다 — 잔존 구간의 출고 일정 지연을 방지할 수 있습니다.';
      } else {
        prediction += ' 잔존 셔플링이 없습니다 — 전체 지시가 원활히 처리되어 후속 작업 대기 없이 이어질 수 있습니다.';
      }
    } else {
      prediction = '지시 ' + fmt(st.dispatch) + '대 중 ' + fmt(dispatchedOut) + '대 출고 · 지시 셔플링 ' + fmt(st.shuffleTotal) +
        '건 중 ' + fmt(st.resolved) + '건 해소(해소율 ' + st.rate + '%) · 작업 진행 중입니다 — 현재 페이스면 잔여 지시가 약 ' + fmt(st.dispatch - st.progress) + '대 남아 있습니다.';
    }
    ...
    if(topCell && st.glifterState.length){
      ...
      const pctUp = st.shuffleTotal > 0 ? Math.round(topCell.count / st.shuffleTotal * 100) : 0;
      const costMin = topCell.count * 5;
      if(g.cell === topCell.id){
        recommendation = g.id + '은 현재 ' + topCell.id + '에 위치해 있습니다. 해당 셀의 잔존 셔플링 ' + fmt(topCell.count) + '건을 해소하면 해소율이 ' + pctUp + '%p 상승합니다 — 약 ' + fmt(costMin) + '분(5분/건)이면 완료되는 규모입니다.';
      } else {
        recommendation = g.id + '은 현재 ' + g.cell + '에 있습니다. 잔존 셔플링이 가장 많은 ' + topCell.id +
          '로 이동하면 해소 ' + fmt(topCell.count) + '건, 해소율이 ' + pctUp + '%p 상승합니다 — 소요 시간 ' + fmt(costMin) + '분 대비 해소 효과가 커 운영 효율성이 높은 이동입니다.';
      }
    } else {
